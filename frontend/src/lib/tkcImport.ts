...
/** 和田委託費の定額(円/月)。額が変わったらここを更新
 *  ★**毎月あるとは限らない**(facts F15)。これは和田を扶養内に収めるための調整弁で、
 *    給与と合わせた総額が65,000に届かない月・少し超える月は**その月は委託費なしで行く**。
 *    実例=2026/04は委託ゼロ(振込は3月分給与95,150の1本のみ・TKC 6117にも守口の計上なし)。
 *    → 「毎月65,000があるはず」という欠落検知を足さないこと(今道のF9と同じ性質)。
...
export function classifyOutsourcing(trader: string, memo: string, storeId?: number, amount?: number): OutsourcingKind {
  ...
  //   ★★読みは「イマミチ」ではなく「コンドウ」(Mneme emp84 name_kana=ｺﾝﾄﾞｳ ﾋｻｺ / 銀行明細の
  //     振込先名も「コンドウ　ヒサコ」)。漢字だけ見て イマミチ と決めつけない。
  //     Mneme全従業員に他の コンドウ/近藤 は居ないため「コンドウ」単独で一意に判定できる。
  //   商流は Smiley→Twinkle→今道 だが、振込は Smiley→今道 の直接。よって銀行明細には
  //   本人名が出るが、TKC計上時に取引先が "Twinkle" 名義に寄せられるリスクが和田と同型にある。
