...
export function classifyOutsourcing(trader: string, memo: string, storeId?: number, amount?: number): OutsourcingKind {
  ...
  // ★「リアライズ」= 今道への支払い経路(Smiley→リアライズ→今道)。TKC上の取引先名は
  //   「リアライズ」で摘要は「委託販売手数料」となり、今道の名前がどこにも出ない可能性が高い。
  //   リアライズは配食(001)・訪問介護(003)の外注先でもあるが、2025年実績で美容部門(011/012)に
  //   リアライズ行は存在しない = うりぼーが扱う美容部門でリアライズが出たら今道分と断定してよい。
  //   (摘要への「今道」明記が徹底されない前提の保険。和田で2度再発した教訓と同型)
  if (/リアライズ|ﾘｱﾗｲｽﾞ|realize/i.test(s)) return 'itaku'
