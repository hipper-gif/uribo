          ...
          <table className="min-w-full text-xs border-collapse">
            <thead>
              <tr className="bg-gray-100">
                <th className="sticky left-0 bg-gray-100 px-2 py-2 text-left border border-gray-200 min-w-[120px]">科目</th>
                {q.months.map(m => (
              ...
              <tr className="bg-gray-50">
                <th className="sticky left-0 bg-gray-50 border border-gray-200" />
                {[...q.months, 0].map((m, i) => (
                  <><th key={`${m}-${i}-t`} className="px-1 py-1 text-right text-[10px] border border-gray-200 text-blue-600">目標</th><th key={`${m}-${i}-a`} className="px-1 py-1 text-right text-[10px] border border-gray-200 text-gray-700">実績</th><th key={`${m}-${i}-r`} className="px-1 py-1 text-right text-[10px] border border-gray-200 text-gray-500">達成率</th></>
                ))}
              </tr>
            </thead>
            <tbody>
              {displayItems.map(item => {
                ...
                return (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="sticky left-0 bg-white px-2 py-1.5 border border-gray-200 font-medium">{item.item_name}</td>
                    {q.months.map(m => {
                      const t = lookup(targetData, item.id, m)
                      const a = lookup(actualData, item.id, m)
                      const rate = t ? a / t : 0
                      return (
                        <><td key={`${m}-t`} className="px-1 py-1.5 text-right border border-gray-200 tabular-nums text-blue-600">{t ? formatAmount(Math.round(t)) : '-'}</td><td key={`${m}-a`} className="px-1 py-1.5 text-right border border-gray-200 tabular-nums">{a ? formatAmount(Math.round(a)) : '-'}</td><td key={`${m}-r`} className={`px-1 py-1.5 text-right border border-gray-200 tabular-nums ${rate >= 1 ? 'text-green-600' : rate > 0 ? 'text-red-500' : ''}`}>
              ...
              <tr className="bg-green-50 font-semibold border-t-2 border-gray-400">
                ...
                {q.months.map(m => {
                  ...
                  return (
                    <><td key={`${m}-t`} className="px-1 py-1.5 text-right border border-gray-200 tabular-nums text-blue-600">{tSales ? formatAmount(Math.round(tProfit)) : '-'}</td><td key={`${m}-a`} className={`px-1 py-1.5 text-right border border-gray-200 tabular-nums ${aProfit < 0 ? 'text-red-600' : ''}`}>{aSales ? formatAmount(Math.round(aProfit)) : '-'}</td><td key={`${m}-r`} className={`px-1 py-1.5 text-right border border-gray-200 tabular-nums ${rate >= 1 ? 'text-green-600' : rate > 0 ? 'text-red-500' : ''}`}>
