...
- ★**「和田」明記は徹底されていない前提で扱う**(2026/06 も明記なしで再発)。`classifyOutsourcing` は**守口(012)のTwinkle名義で本人名(杉原/爽夏/サヤカ)を含まない行を和田委託と自動判定**する(真のTwinkle代は「W21 テインクル,スギハラ サヤカ」のように本人名入りで振り込まれる)。自動判定が外れた場合は、TKCインポートのプレビューで**6117明細ごとに「Twinkle代/和田委託/その他外注」をプルダウンで手動修正**できる(2026-07-16 追加)。
