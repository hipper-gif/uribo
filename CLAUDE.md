...
- ★**「和田」明記は徹底されていない前提で扱う**(2026/06 も明記なしで再発)。`classifyOutsourcing` の自動判定(優先順): ①「和田」明記→和田 ②**金額が定額65,000円(`WADA_FIXED_FEE`)→和田**(名義不問。和田の委託費は定額のため一番ブレない判定。額が変わったら定数を更新) ③守口(012)のTwinkle名義で本人名(杉原/爽夏/サヤカ)なし→和田(真のTwinkle代は「W21 テインクル,スギハラ サヤカ」のように本人名入り)。自動判定が外れた場合は、TKCインポートのプレビューで**6117明細ごとに「Twinkle代/和田委託/その他外注」をプルダウンで手動修正**できる(2026-07-16 追加)。
